...
const TABLES = {
  ...
  adoptionApplications: "tblmDw2mrdg40JyoF",
  adoptions: "tblssnhDEIkz2D0iY",
  ...
  return {
    ...
    inventory: inventory
      ...
      .map((record) => ({
        ...
        vendor: asText(record.fields["Preferred Vendor"]),
        purchaseUrl: asText(record.fields["Purchase URL"]),
        unitCost: asNumber(record.fields["Typical Unit Cost"]),
        responsiblePerson: asText(record.fields["Responsible Person"]),
        responsibleEmail: asText(record.fields["Responsible Email"]),
        trackLotExpiration: Boolean(record.fields["Track Lot / Expiration"]),
        notes: asText(record.fields.Notes),
