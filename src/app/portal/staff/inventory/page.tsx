  ...
  return <main className="min-h-screen bg-slate-50"><div className="container-custom py-10 md:py-12">
    ...
    <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
      ...
      <div className="space-y-6">
        <section className="rounded-3xl border bg-white p-6 shadow-sm md:p-8"><div className="mb-5 flex items-center gap-3"><PackagePlus className="h-6 w-6 text-primary"/><div><h2 className="text-2xl font-bold">Record Inventory Movement</h2><p className="text-sm text-muted-foreground">Current quantity is calculated from these transactions.</p></div></div><form action={recordTransaction} className="space-y-3"><select name="itemId" required defaultValue="" className="w-full rounded-xl border bg-white px-3 py-2.5"><option value="" disabled>Select shelter item</option>{active.map(r=><option key={r.id} value={r.id}>{asText(r.fields["Item Name"])}</option>)}</select><select name="type" required defaultValue="Received" className="w-full rounded-xl border bg-white px-3 py-2.5">{["Received","Used","Adjustment +","Adjustment -","Damaged / Discarded","Expired"].map(x=><option key={x}>{x}</option>)}</select><input name="quantity" type="number" min="0.01" step="0.01" required placeholder="Quantity" className="w-full rounded-xl border px-3 py-2.5"/><input name="vendor" placeholder="Vendor / source" className="w-full rounded-xl border px-3 py-2.5"/><input name="lotNumber" placeholder="Lot number" className="w-full rounded-xl border px-3 py-2.5"/><input name="expirationDate" type="date" className="w-full rounded-xl border px-3 py-2.5"/><input name="cost" type="number" min="0" step="0.01" placeholder="Cost" className="w-full rounded-xl border px-3 py-2.5"/><textarea name="notes" rows={3} placeholder="Notes" className="w-full rounded-xl border px-3 py-2.5"/><button className="w-full rounded-full bg-primary px-5 py-3 font-semibold text-white">Save Transaction</button></form></section>
