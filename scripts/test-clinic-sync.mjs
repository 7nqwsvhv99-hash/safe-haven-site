...
async function run(db,failAfter=Infinity){
 ...
 try{return {response:await sync(),writes};}finally{global.fetch=oldFetch;global.setTimeout=oldTimeout;}
}
...
test('source date change invalidates commitments, holds bookings and queues notices once',async()=>{
 ...
 assert.equal(db[D][0].fields['Clinic Date'],moved);assert.equal(db[R][0].fields['Initial Response'],'No Response');assert.equal(db[R][0].fields['Reconfirmation Date'],null);assert.equal(db[D][0].fields['Booked Appointment Follow-Up Required'],true);assert.equal(db[CD][0].fields['Scheduling Hold?'],true);assert.equal(count,3);assert.equal(db[Q].length,count);
});
test('local date change preserves booked patient date until coordinator clears follow-up',async()=>{
 ...
 db[D][0].fields['Booked Appointment Follow-Up Required']=false;await run(db);assert.equal(db[CD][0].fields.Clinic_Date,moved);
});
...
test('unchanged sync performs no Airtable writes',async()=>{const db=fixture();await run(db);const result=await run(db);assert.equal(result.writes,0);});
